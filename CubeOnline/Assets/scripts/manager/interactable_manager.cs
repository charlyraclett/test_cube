...
public class interactable_manager : MonoBehaviour{
    ...
    public void player_is_in_area_interactable(object _interactable){
        ...
        if(!player_manager.inst.my_avatar.host)
        return;
    ...
    public void press_for_action(){
        ...
        if(object_interactable != null){
            ...
            if(object_interactable.GetType() == typeof(elevator)){
                do_action_elevator((elevator)object_interactable);   
            }
    ...
    void do_action_elevator(elevator _elevator){

        StartCoroutine(_elevator.up_elevator());
    ...
    void change_and_show_text_button(object _interactable){
        ...
        if(_interactable.GetType() == typeof(switch_light)){
            text_button_action = "Activate";
        }

        if(_interactable.GetType() == typeof(elevator)){
            text_button_action = "Up";
        }
